...
foreach ($Computer in $Computers)
    ...
    if ($fixable)
        ...
        $Empty = $False
        $USU = Invoke-Command -ComputerName $Computer -ScriptBlock {Get-LocalUser -Name USU}
        $Administrator = Invoke-Command -ComputerName $Computer -ScriptBlock {Get-LocalUser -Name Administrator}
        $LastPasswordSet = $USU.PasswordLastSet
        $Difference = $Today - $LastPasswordSet
        if ($Difference.Days -gt 14)
            {
            $Days = $Difference.Days
            Write-Log "Geater than 14 days, $Days days"
            Write-host "Last password change: " $LastPasswordSet
            $Empty = $False
            ...
            {
            Write-Log "Lower than 14 days"
            Write-host "Last password change: " $LastPasswordSet
            }
        if ($Administrator.Enabled)
            {
            Write-Log "Administrator is enabled, disabling"
            Invoke-Command -ComputerName $Computer -ScriptBlock {Disable-LocalUser -Name Administrator}
            if ($Administrator.Enabled)
                {
                Write-Log "Administrator is still enabled"
                Write-Host "Administrator is still enabled"
                }
                else
                {
                Write-Log "Administrator is disabled"
                }
            }
            else
            {
