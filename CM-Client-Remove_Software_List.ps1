...
foreach ($Computer in $Computers)
    ...
    if ($fixable)
        ...
        Invoke-Command -ComputerName $Computer -ScriptBlock {
            ...
            # List of MSI product codes
            $msiCodes = @(
                "{ac76ba86-7ad7-1033-7b44-ac0f074e4100}"
            )

            # Retrieve all installed products once
            $installedProducts = Get-CimInstance -ClassName Win32_Product

            # Loop through each MSI code and check if it's installed
            foreach ($code in $msiCodes)
                {
                $product = $installedProducts | Where-Object { $_.IdentifyingNumber -eq $code }
                if ($product) {$product | Invoke-CimMethod -MethodName Uninstall; Write-Host "Uninstalled: $($product.Name)" -ForegroundColor Red}
                }
            
            try {Invoke-WmiMethod -Namespace root\CCM -Class SMS_Client -Name TriggerSchedule -ArgumentList $HardwareInventoryID -ErrorAction Stop; Write-Host "Successfully triggered new Hardware Inventory"}
            catch{Write-Host "Failed to trigger new Hardware Inventory"}
            }
        Write-Log "Finished"
	    }
    }
